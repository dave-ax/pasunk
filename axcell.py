...
def log(value, status='', color=''):
    value = colors('{color}[{time}] [CC]START {color}{status} [CC]{color}{value}[CC]'.format(
...
class inject(object):
    ...
    def start(self):
        try:
            ...
            frontend_domains = open(real_path('/ac.txt')).readlines()
            frontend_domains = filter_array(frontend_domains)
            if len(frontend_domains) == 0:
                self.log('Frontend Domains not found. Please check ac.txt', color='G1')
                return
            ...
            self.log('SILAHKAN ULANGI'.format(self.inject_host, self.inject_port), color='[R1]')
...
class domain_fronting(threading.Thread):
    ...
    def log(self, value, status='RX|TX', 
    color=''):
    ...
    def run(self):
        try:
            ...
            self.log('[CC]00'.format(self.proxy_host, self.proxy_port))
            ...
            self.log('11'.format(self.proxy_host, self.proxy_port), color='[G1]')
        except OSError:
...
print(colors('\n'.join([
        '[R1]AX|NET','[CC]'
        '[R1]REG 16060963908899','[CC]'
        '[R1]Host : 127.0.0.1 | Port : 1945'
    ])))
def main():
    
    inject('127.0.0.1', '1945').start()
